...
<!--      <el-table-column label="1-有效 0-无效" align="center" prop="status" />-->
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        status: [
          { required: true, message: "1-有效 0-无效不能为空", trigger: "change" }
        ]
      }
    }
  },
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除模型数据列编号为"' + ids + '"的数据项？').then(function() {
        return delMODEL(ids)
      }).then(() => {
        this.getList()
